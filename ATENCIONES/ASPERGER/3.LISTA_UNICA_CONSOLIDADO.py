...
def procesar_datos():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS mtd_lista_unica_pacientes_asperger(
    ...
    for row in combined_df.iter_rows(named=True):
        ...
        cursor.execute(f"""
            INSERT INTO mtd_lista_unica_pacientes_asperger ({", ".join(COLUMN_ORDER)}) 
