...
# Diagnósticos de osteoporosis a filtrar
DIAGCOD_OSTEOPOROSIS = ('M80%', 'M81%', 'M82%')  # Uso de LIKE con '%'
        ...
        for año in ANIOS_PERMITIDOS:
            ...
            for i in range(len(TABLAS_ORIGEN)):
                ...
                query_select = f"""
                    ...
                    FROM {tabla_origen}
                    WHERE (DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s)
                    AND anio = %s
                """

                # Leer los datos para el año actual
                df = pd.read_sql(query_select, conn, params=(*DIAGCOD_OSTEOPOROSIS, año))
