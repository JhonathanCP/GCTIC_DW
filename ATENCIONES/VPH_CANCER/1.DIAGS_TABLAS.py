...
# Diagnósticos de cancer_vph a filtrar
DIAGCOD_CANCER_VPH = ('C10%', 'C21%', 'C51%', 'C52%','C53%','C60%','D06%', 'N87%') # Uso de LIKE con '%'
        ...
        for año in ANIOS_PERMITIDOS:
            ...
            for i in range(len(TABLAS_ORIGEN)):
                ...
                query_select = f"""
                    ...
                    FROM {tabla_origen}
                    WHERE (DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s OR DIAGCOD LIKE %s)
                    AND anio = %s
                """

                # Leer los datos para el año actual
                df = pd.read_sql(query_select, conn, params=(*DIAGCOD_CANCER_VPH, año))
